        ...
        >
          <div class="middle-l" ref="middleL">
            <div class="cd-wrapper" ref="cdWrapper">
              <div class="cd">
                <img class="image" :class="cdClass" :src="currentSong.image" alt="">
              </div>
            </div>
      ...
      <div class="mini-player" @click="open" v-show="!fullScreen">
        <div class="icon">
          <div class="imgWrapper" ref="miniWrapper">
            <img width="40" height="40" :class="cdClass" :src="currentSong.image" alt="">
          </div>
        </div>
        <div class="text">
...
export default {
  ...
  methods: {
    ...
    // vue transition中的JavaScript钩子
    enter(el, done) {
      const {x, y, scale} = this._getPosAndScale()
      // 用js创建css动画
      let animation = {
        0: {
          // 一开始mini唱片的位置与缩放大小，接下来就要从该状态过渡到原本normal唱片的位置与大小
    ...
    handlerLyric({lineNum, txt}) {
      ...
      if (lineNum > 5) {
        ...
        this.$refs.lyricList.scrollToElement(lines[lineNum - 5], 1000)
      } else {
    ...
    middleTouchEnd(e) {
      ...
      this.$refs.lyricList.$el.style[transform] = `translate3d(${offsetWidth}px, 0, 0)`
      this.$refs.lyricList.$el.style[transitionDuration] = `600ms`
      this.$refs.middleL.style[transitionDuration] = `600ms`
    ...
    // 获取normal播放器中唱片对应下方mini播放器唱片的位置
    _getPosAndScale() {
      ...
      const scale = targetWidth / width // mini唱片相较于normal唱片缩放比例
      const x = -(window.innerWidth / 2 - paddingLeft) // normal与mini的位置 x，y距离都是计算求得
